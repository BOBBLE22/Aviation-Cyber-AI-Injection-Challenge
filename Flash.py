...
LEVELS = {
    "1": {
        ...
        "fallbacks": [
            {"provider": "nvidia",      "model": "meta/llama3-8b-instruct"},
...
def call_openrouter(model, system, message):
...
def chat(level_id):
    ...
    if last_error:
        if "429" in last_error or "quota" in last_error.lower() or "rate" in last_error.lower():
            return jsonify({"reply": "[SYSTEM ALERT] Sector relay congested. Please wait 10 seconds and retry your injection."})
